import { test, moduleFor } from 'ember-qunit';
import Bill from 'sky-bill/models/bill';


var billjson = {
		  "statement": {
		    "generated": "2015-01-11",
		    "due": "2015-01-25",
		    "period": {
		      "from": "2015-01-26",
		      "to": "2015-02-25"
		    }
		  },
		  "total": 136.03,
		  "package": {
		    "subscriptions": [
		      { "type": "tv", "name": "Variety with Movies HD", "cost": 50.00 },
		      { "type": "talk", "name": "Sky Talk Anytime", "cost": 5.00 },
		      { "type": "broadband", "name": "Fibre Unlimited", "cost": 16.40 }
		    ],
		    "total": 71.40
		  },
		  "callCharges": {
		    "calls": [
		      { "called": "07716393769", "duration": "00:23:03", "cost": 2.13 },
		      { "called": "07716393769", "duration": "00:23:03", "cost": 2.13 },
		      
		    ],
		    "total": 59.64
		  },
		  "skyStore": {
		    "rentals": [
		      { "title": "50 Shades of Grey", "cost": 4.99 }
		    ],
		    "buyAndKeep": [
		      { "title": "That's what she said", "cost": 9.99 },
		      { "title": "Broke back mountain", "cost": 9.99 }
		    ],
		    "total": 24.97
		  }
		};


var billjsonWrong = {
		  "statement": {
		  },
		  "total": 136.03,
		  "package": {
		    "subscriptions": [
		    ],
		    "total": 71.40
		  },
		  "callCharges": {
		  },
		  "skyStore": {
		  }
};





moduleFor('model:bill', "Unit - Bill", {
// only neccessary if you want to load other items into the runtime
// needs: ['controller:index']
beforeEach: function () {},
afterEach: function () {}
});

test("bill properties exist", function(assert){
	var b = Bill.create(billjson);
	assert.ok(b.get("hasTv"));
	assert.ok(b.get("hasTalk"));
	assert.ok(b.get("hasBroadband"));
});


test("bill properties don't exist", function(assert){
	var b = Bill.create(billjsonWrong);
	assert.ok(!b.get("hasTv"));
	assert.ok(!b.get("hasTalk"));
	assert.ok(!b.get("hasBroadband"));
});





